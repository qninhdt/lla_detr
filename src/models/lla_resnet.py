...
def conv3x3(layer: nn.Module, in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1, num_embeddings: int = 1) -> nn.Module:
...
def conv1x1(layer: nn.Module, in_planes: int, out_planes: int, stride: int = 1, num_embeddings: int = 1) -> nn.Module:
    """1x1 convolution"""
    if layer is LLAConv2d:
        return layer(
            in_planes, out_planes, kernel_size=(1,1), stride=stride, num_embeddings=num_embeddings
        )
    else:
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv3x3(layer, width, width, stride, groups, dilation, num_embeddings=num_embeddings)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(layer, width, planes * self.expansion, num_embeddings=num_embeddings)
    ...
    **kwargs: Any,
    ...
    if weights is not None:
        ...
        for k, v in state_dict.items():
            if k in targets:
                state_dict_[k.replace("weight", "kernel_embed")] = v.unsqueeze(0).repeat(num_embeddings, 1, 1, 1, 1)
                del state_dict_[k]
